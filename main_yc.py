...
from ycProcess import ycProcess
...
plt.title(
    f'Average loss of PCA and Fair PCA on Yield Curve data', fontsize=20)
plt.legend(['Inverted/other yield curve loss PCA',
           'Normal yield curve loss PCA', 'Fair loss'], fontsize=15)
plt.xlabel('Number of dimensions', fontsize=20)
plt.ylabel('Loss', fontsize=20)
plt.savefig(f'figs/yc_loss.jpg', dpi=300)
...
plt.title(
    'Average reconstruction error (RE) of PCA and Fair PCA on Yield Curve data', fontsize=20)
plt.legend(['Normal yield curve RE PCA', 'Inverted/other yield curve RE PCA', 'Normal yield curve RE Fair PCA',
           'Inverted/other yield curve RE Fair PCA'], fontsize=15)
plt.xlabel('Number of features', fontsize=20)
plt.ylabel('Average reconstruction error (ARE)', fontsize=20)
plt.savefig(f'figs/yc_re.jpg', dpi=300)
...
res_loss.to_csv(f'tables/yc_loss_table.csv')
...
res_re.to_csv(f'tables/yc_re_table.csv')
